...
async fn tick_once(
    ...
    // Plain `retire()`, not the PID-based tree kill: on Windows every supervised
    // child is assigned to a job carrying KILL_ON_JOB_CLOSE, so the drain's
    // existing `start_kill` already terminates the whole tree. `retire_tree()`
    // would add a second mechanism that races the parent exiting.
    for module in supervisor.list() {
        match module.retire().await {
            Ok(()) => {
                supervisor.retire(module.module_id());
            }
            Err(error) => {
                warn!(%error, "holder monitor: module retirement failed");
                complete = false;
            }
        }
    }
